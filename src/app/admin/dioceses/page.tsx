...
import {
  ...
  updateDiocese,
  deleteDiocese,
  getDioceseChurchesCount,
} from '@/lib/sunday-school/dioceses'
import type { Diocese, CreateDioceseInput } from '@/lib/types/sunday-school'

export default function DiocesesPage() {
  const [dioceses, setDioceses] = useState<Diocese[]>([])
  const [churchCounts, setChurchCounts] = useState<Record<string, number>>({})
  ...
  async function loadDioceses() {
    try {
      ...
      // Load church counts for each diocese
      const counts: Record<string, number> = {}
      await Promise.all(
        data.map(async (diocese) => {
          counts[diocese.id] = await getDioceseChurchesCount(diocese.id)
        })
      )
      setChurchCounts(counts)
    } catch (error) {
      console.error('Error loading dioceses:', error)
      toast.error('Failed to load dioceses')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editingDiocese) {
        await updateDiocese(editingDiocese.id, formData)
        toast.success('Diocese updated successfully')
      } else {
        await createDiocese(formData)
